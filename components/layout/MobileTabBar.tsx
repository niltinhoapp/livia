...
export function MobileTabBar() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-primary-100 bg-white/95 shadow-[0_-4px_18px_rgba(124,58,237,0.08)] backdrop-blur dark:border-ink-700 dark:bg-ink-900/95 lg:hidden">
      {items.map((item) => {
        const active = item.href === "/painel" ? pathname === item.href : pathname === item.href || pathname.startsWith(`${item.href}/`);
