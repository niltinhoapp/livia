...
const listMenuCategoryTool: ToolDefinition = { name: "list_menu_category", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("list_menu_category", "Lista produtos disponíveis de UMA categoria do cardápio (ex.: pizzas, bebidas, lanches). Use quando o cliente pedir uma categoria específica; não invente itens ou preços.", { type: "object", properties: { category: { type: "string" } }, required: ["category"] }), async execute(ctx, args) { const raw = typeof args.category === "string" ? args.category.trim() : ""; const query = raw.toLocaleLowerCase("pt-BR"); if (!query) return { ok: false, error: "categoria obrigatória" }; const category = (await (await orderService()).listAvailableMenu(ctx.est.id)).find((item) => item.id === raw || item.name.toLocaleLowerCase("pt-BR") === query); if (!category) return { ok: false, error: "categoria não encontrada" }; return { ok: true, data: { id: category.id, name: category.name, products: category.products.map((p) => ({ id: p.id, name: p.name, description: p.description, basePriceCents: p.basePriceCents, variants: p.variants.filter((v) => v.active), modifierGroups: p.modifierGroups.map((g) => ({ id: g.id, name: g.name, required: g.required, minSelections: g.minSelections, maxSelections: g.maxSelections, options: g.options.filter((o) => o.active) })) })) } }; } };
