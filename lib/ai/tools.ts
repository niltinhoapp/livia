...
function orderSummary(order: FoodOrder | null) {
  if (!order) return null;
  return { id: order.id, status: order.status, version: order.version, items: order.items.map((item) => ({ id: item.id, name: item.productName, variant: item.variantName, quantity: item.quantity, modifiers: item.modifiers.map((m) => m.name), notes: item.notes, lineTotalCents: item.lineTotalCents })), fulfillment: order.fulfillment, deliveryAddress: order.deliveryAddress, payment: order.payment, subtotalCents: order.subtotalCents, deliveryFeeCents: order.deliveryFeeCents, totalCents: order.totalCents };
}
const orderConversationId = (ctx: ToolContext) => normalizePhone(ctx.contactPhone);
const operationIdFor = (ctx: ToolContext, _tool: string) => ctx.operationId;
const searchMenu: ToolDefinition = { name: "search_menu", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("search_menu", "Busca produtos DISPONÍVEIS no cardápio real. Use antes de adicionar itens ou informar preço; nunca invente produto, adicional ou valor.", { type: "object", properties: { query: { type: "string" } }, required: ["query"] }), async execute(ctx, args) { const query = typeof args.query === "string" ? args.query.trim().toLocaleLowerCase("pt-BR") : ""; if (!query) return { ok: false, error: "query obrigatória" }; const products = (await (await orderService()).listMenuProducts(ctx.est.id)).filter((p) => p.active && `${p.name} ${p.description ?? ""}`.toLocaleLowerCase("pt-BR").includes(query)).slice(0, 12); return { ok: true, data: { products: products.map((p) => ({ id: p.id, name: p.name, description: p.description, basePriceCents: p.basePriceCents, variants: p.variants.filter((v) => v.active), modifierGroups: p.modifierGroups.map((g) => ({ id: g.id, name: g.name, required: g.required, minSelections: g.minSelections, maxSelections: g.maxSelections, options: g.options.filter((o) => o.active) })) })) } }; } };
const getMenuProductTool: ToolDefinition = { name: "get_menu_product", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("get_menu_product", "Retorna composição e preço REAIS de um produto pelo id recebido em search_menu.", { type: "object", properties: { productId: { type: "string" } }, required: ["productId"] }), async execute(ctx, args) { const product = typeof args.productId === "string" ? await (await orderService()).getMenuProduct(ctx.est.id, args.productId) : null; return product?.active ? { ok: true, data: product } : { ok: false, error: "produto indisponível" }; } };
const getOrderDraftTool: ToolDefinition = { name: "get_order_draft", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("get_order_draft", "Retorna o resumo canônico do pedido atual, incluindo total calculado pelo backend. Use antes de apresentar total ou pedir confirmação.", { type: "object", properties: {} }), async execute(ctx) { return { ok: true, data: orderSummary(await (await orderService()).getActiveOrder(ctx.est.id, orderConversationId(ctx))) }; } };
const addOrderItemTool: ToolDefinition = { name: "add_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("add_order_item", "Adiciona produto ao pedido usando IDs de search_menu/get_menu_product. O backend valida disponibilidade, variação, adicionais e preço.", { type: "object", properties: { productId: { type: "string" }, variantId: { type: "string" }, modifierOptionIds: { type: "array", items: { type: "string" } }, quantity: { type: "number" }, notes: { type: "string" } }, required: ["productId", "quantity"] }), async execute(ctx, args) { try { const order = await (await orderService()).addOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.productId ?? ""), typeof args.variantId === "string" ? args.variantId : null, Array.isArray(args.modifierOptionIds) ? args.modifierOptionIds.filter((v): v is string => typeof v === "string") : [], Number(args.quantity), typeof args.notes === "string" ? args.notes : null, operationIdFor(ctx, "add_order_item")); return { ok: true, data: orderSummary(order) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const updateOrderItemTool: ToolDefinition = { name: "update_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("update_order_item", "Altera a quantidade ou observação de um item do pedido. Use o itemId retornado pelo resumo.", { type: "object", properties: { itemId: { type: "string" }, quantity: { type: "number" }, notes: { type: "string" } }, required: ["itemId"] }), async execute(ctx, args) { try { const order = await (await orderService()).updateOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.itemId ?? ""), { quantity: typeof args.quantity === "number" ? args.quantity : undefined, notes: typeof args.notes === "string" ? args.notes : undefined }, operationIdFor(ctx, "update_order_item")); return { ok: true, data: orderSummary(order) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const removeOrderItemTool: ToolDefinition = { name: "remove_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("remove_order_item", "Remove um item identificado pelo itemId do resumo atual.", { type: "object", properties: { itemId: { type: "string" } }, required: ["itemId"] }), async execute(ctx, args) { try { return { ok: true, data: orderSummary(await (await orderService()).removeOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.itemId ?? ""), operationIdFor(ctx, "remove_order_item"))) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderFulfillmentTool: ToolDefinition = { name: "set_order_fulfillment", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_fulfillment", "Define retirada ou entrega. Nunca estime taxa: o backend calcula.", { type: "object", properties: { fulfillment: { type: "string", enum: ["pickup", "delivery"] } }, required: ["fulfillment"] }), async execute(ctx, args) { try { const fulfillment = args.fulfillment === "pickup" || args.fulfillment === "delivery" ? args.fulfillment : null; if (!fulfillment) throw new Error("modalidade inválida"); return { ok: true, data: orderSummary(await (await orderService()).setOrderFulfillment(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, fulfillment, operationIdFor(ctx, "set_order_fulfillment"))) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderAddressTool: ToolDefinition = { name: "set_order_address", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_address", "Registra o endereço informado pelo cliente para entrega. Pergunte bairro se houver taxa por bairro e não invente dados.", { type: "object", properties: { raw: { type: "string" }, neighborhood: { type: "string" }, reference: { type: "string" } }, required: ["raw"] }), async execute(ctx, args) { try { return { ok: true, data: orderSummary(await (await orderService()).setOrderAddress(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.raw ?? ""), typeof args.neighborhood === "string" ? args.neighborhood : null, typeof args.reference === "string" ? args.reference : null, operationIdFor(ctx, "set_order_address"))) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderPaymentTool: ToolDefinition = { name: "set_order_payment", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_payment", "Define a forma de pagamento aceita pelo estabelecimento. Não processa pagamento nem usa billing SaaS.", { type: "object", properties: { method: { type: "string", enum: ["pix", "cash", "credit_card", "debit_card"] }, changeForCents: { type: "number" } }, required: ["method"] }), async execute(ctx, args) { try { const method = args.method as OrderPaymentMethod; if (!["pix", "cash", "credit_card", "debit_card"].includes(method)) throw new Error("forma inválida"); return { ok: true, data: orderSummary(await (await orderService()).setOrderPayment(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, method, typeof args.changeForCents === "number" ? args.changeForCents : null, operationIdFor(ctx, "set_order_payment"))) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const confirmOrderTool: ToolDefinition = { name: "confirm_order", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("confirm_order", "Confirma o pedido somente após confirmação explícita do cliente e resumo canônico. Use orderId e version retornados por get_order_draft.", { type: "object", properties: { orderId: { type: "string" }, version: { type: "number" } }, required: ["orderId", "version"] }), async execute(ctx, args) { try { const order = await (await orderService()).confirmOrder(ctx.est.id, String(args.orderId ?? ""), Number(args.version), ctx.contactPhone); return { ok: true, data: orderSummary(order) }; } catch (e) { return { ok: false, error: String(e) }; } } };
...
export const TOOL_REGISTRY: ToolDefinition[] = [
  ...
  rescheduleAppointment,
  cancelAppointment,
  searchMenu, getMenuProductTool, getOrderDraftTool, addOrderItemTool, updateOrderItemTool, removeOrderItemTool, setOrderFulfillmentTool, setOrderAddressTool, setOrderPaymentTool, confirmOrderTool, getOrderStatusTool,
  requestHumanHandoff,
];
