...
const addOrderItemTool: ToolDefinition = { name: "add_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("add_order_item", "Adiciona produto ao pedido usando IDs de search_menu/get_menu_product. O backend valida disponibilidade, variação, adicionais e preço.", { type: "object", properties: { productId: { type: "string" }, variantId: { type: "string" }, modifierOptionIds: { type: "array", items: { type: "string" } }, quantity: { type: "number" }, notes: { type: "string" } }, required: ["productId", "quantity"] }), async execute(ctx, args) { try { const order = await (await orderService()).addOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.productId ?? ""), typeof args.variantId === "string" ? args.variantId : null, Array.isArray(args.modifierOptionIds) ? args.modifierOptionIds.filter((v): v is string => typeof v === "string") : [], Number(args.quantity), typeof args.notes === "string" ? args.notes : null); return { ok: true, data: orderSummary(order) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const updateOrderItemTool: ToolDefinition = { name: "update_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("update_order_item", "Altera a quantidade ou observação de um item do pedido. Use o itemId retornado pelo resumo.", { type: "object", properties: { itemId: { type: "string" }, quantity: { type: "number" }, notes: { type: "string" } }, required: ["itemId"] }), async execute(ctx, args) { try { const order = await (await orderService()).updateOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.itemId ?? ""), { quantity: typeof args.quantity === "number" ? args.quantity : undefined, notes: typeof args.notes === "string" ? args.notes : undefined }); return { ok: true, data: orderSummary(order) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const removeOrderItemTool: ToolDefinition = { name: "remove_order_item", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("remove_order_item", "Remove um item identificado pelo itemId do resumo atual.", { type: "object", properties: { itemId: { type: "string" } }, required: ["itemId"] }), async execute(ctx, args) { try { return { ok: true, data: orderSummary(await (await orderService()).removeOrderItem(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.itemId ?? ""))) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderFulfillmentTool: ToolDefinition = { name: "set_order_fulfillment", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_fulfillment", "Define retirada ou entrega. Nunca estime taxa: o backend calcula.", { type: "object", properties: { fulfillment: { type: "string", enum: ["pickup", "delivery"] } }, required: ["fulfillment"] }), async execute(ctx, args) { try { const fulfillment = args.fulfillment === "pickup" || args.fulfillment === "delivery" ? args.fulfillment : null; if (!fulfillment) throw new Error("modalidade inválida"); return { ok: true, data: orderSummary(await (await orderService()).setOrderFulfillment(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, fulfillment)) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderAddressTool: ToolDefinition = { name: "set_order_address", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_address", "Registra o endereço informado pelo cliente para entrega. Pergunte bairro se houver taxa por bairro e não invente dados.", { type: "object", properties: { raw: { type: "string" }, neighborhood: { type: "string" }, reference: { type: "string" } }, required: ["raw"] }), async execute(ctx, args) { try { return { ok: true, data: orderSummary(await (await orderService()).setOrderAddress(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, String(args.raw ?? ""), typeof args.neighborhood === "string" ? args.neighborhood : null, typeof args.reference === "string" ? args.reference : null)) }; } catch (e) { return { ok: false, error: String(e) }; } } };
const setOrderPaymentTool: ToolDefinition = { name: "set_order_payment", enabled: (ctx) => Boolean(ctx.est.bot.ordersEnabled), schema: fn("set_order_payment", "Define a forma de pagamento aceita pelo estabelecimento. Não processa pagamento nem usa billing SaaS.", { type: "object", properties: { method: { type: "string", enum: ["pix", "cash", "credit_card", "debit_card"] }, changeForCents: { type: "number" } }, required: ["method"] }), async execute(ctx, args) { try { const method = args.method as OrderPaymentMethod; if (!["pix", "cash", "credit_card", "debit_card"].includes(method)) throw new Error("forma inválida"); return { ok: true, data: orderSummary(await (await orderService()).setOrderPayment(ctx.est.id, orderConversationId(ctx), ctx.contactPhone, ctx.contactName, method, typeof args.changeForCents === "number" ? args.changeForCents : null)) }; } catch (e) { return { ok: false, error: String(e) }; } } };
