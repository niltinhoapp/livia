...
beforeEach(() => { vi.clearAllMocks(); vi.stubEnv("OPENAI_API_KEY", "test-key"); create.mockResolvedValue({ arrayBuffer: async () => new Uint8Array([0x4f, 0x67, 0x67, 0x53, 1]).buffer }); });
describe("synthesizeSpeech", () => {
  ...
  it("rejeita resposta que não seja container Ogg antes do upload", async () => { create.mockResolvedValueOnce({ arrayBuffer: async () => new Uint8Array([1, 2, 3]).buffer }); await expect(synthesizeSpeech("ok")).rejects.toMatchObject({ code: "invalid_audio" }); });
});
